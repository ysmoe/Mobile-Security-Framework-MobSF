...
SEVERITY_COLORS = {
    ...
    # Bootstrap-4 table-cell prefixes used by MobSF PDF templates
    'td-danger': (0xC0, 0x39, 0x2B),
    'td-warning': (0xE6, 0x9D, 0x00),
    'td-info': (0x29, 0x80, 0xB9),
    'td-success': (0x18, 0x9A, 0x4B),
    'td-active': (0x29, 0x80, 0xB9),
    'text-danger': (0xC0, 0x39, 0x2B),
    'text-warning': (0xE6, 0x9D, 0x00),
    'text-info': (0x29, 0x80, 0xB9),
    'text-success': (0x18, 0x9A, 0x4B),
}
...
def _shade_paragraph(para, hex_color):
    """Apply a background fill to a paragraph (used for badge covers)."""
    from docx.oxml import OxmlElement
...
def _set_paragraph_alignment(para, alignment):
    para.alignment = alignment


def _build_cover_page(doc, context):
    """Render a one-page cover that summarises the scan at a glance.

    Layout (A4 portrait, 2cm margins):
        1. MobSF logo (centered) + '移动安全框架' subtitle
        2. Horizontal rule
        3. Big platform heading (e.g. 'Android 静态分析报告')
        4. App icon (centered, 2.5cm wide)
        5. App name + version (24pt centered)
        6. Package name (smaller, centered)
        7. Security score badge (filled colour block) + risk label
        8. Metadata table (file name, scan date, MD5/SHA)
        9. Report footer line (generated by MobSF)
       10. Hard page break to begin the body

    If any field is missing in ``context`` the cover degrades gracefully
    rather than raising.
    """
    if not context:
        return
    ...
    logo = context.get('logo') or context.get('base_url', '') + (
        '/static/img/mobsf_logo.png')
    logo_bytes = _inline_image_bytes(logo)
    if logo_bytes:
        para = doc.add_paragraph()
        para.alignment = 1  # CENTER
        try:
            para.add_run().add_picture(
                io.BytesIO(logo_bytes), width=Cm(4.0))
        except Exception as exc:  # pragma: no cover
            logger.debug('cover: cannot inline logo: %s', exc)

    para = doc.add_paragraph()
    para.alignment = 1
    run = para.add_run('移动安全框架 (MobSF)')
    _set_run_font(run, size=14, bold=True, color=(0x29, 0x80, 0xB9))
    _add_hr(doc)

    # ---- Platform + report type ----
    app_type = (context.get('app_type') or '').lower()
    platform_map = {
        'apk': 'Android 静态分析报告',
        'android': 'Android 静态分析报告',
        'ipa': 'iOS 静态分析报告',
        'ios': 'iOS 静态分析报告',
        'appx': 'Windows 静态分析报告',
        'windows': 'Windows 静态分析报告',
    }
    heading_text = platform_map.get(
        app_type, '移动应用静态分析报告')
    para = doc.add_paragraph()
    para.alignment = 1
    run = para.add_run(heading_text)
    _set_run_font(run, size=22, bold=True, color=(0x1F, 0x4E, 0x79))
    # Some vertical breathing room
    para.paragraph_format.space_before = Pt(18)
    para.paragraph_format.space_after = Pt(12)

    # ---- App icon (centered, 2.5cm) ----
    icon = context.get('icon_path')
    if icon:
        dwd = context.get('dwd_dir', '')
        icon_src = f"{dwd}{icon}" if dwd else icon
        icon_bytes = _inline_image_bytes(icon_src)
        if not icon_bytes:
            icon_bytes = _inline_image_bytes(
                context.get('base_url', '') + '/static/img/no_icon.png')
        if icon_bytes:
            para = doc.add_paragraph()
            para.alignment = 1
            try:
                para.add_run().add_picture(
                    io.BytesIO(icon_bytes), width=Cm(3.0))
            except Exception as exc:  # pragma: no cover
                logger.debug('cover: cannot inline icon: %s', exc)

    # ---- App name + version (centered, large) ----
    app_name = (context.get('app_name') or context.get('file_name')
                or '移动应用').strip()
    version_name = (context.get('version_name') or '').strip()
    para = doc.add_paragraph()
    para.alignment = 1
    ...
    if version_name:
        ...
        para2.alignment = 1
        run = para2.add_run(f'版本 {version_name}')
        _set_run_font(run, size=12, color=(0x66, 0x66, 0x66))
        para2.paragraph_format.space_after = Pt(6)

    # ---- Package / identifier line ----
    pkg = (context.get('package_name')
           or context.get('bundle_id')
           or context.get('binary_id')
           or '').strip()
    if pkg:
        para = doc.add_paragraph()
        para.alignment = 1
        run = para.add_run(pkg)
        _set_run_font(run, size=11, color=(0x66, 0x66, 0x66))

    # ---- Security score badge ----
    appsec = context.get('appsec') or {}
    score = appsec.get('security_score') if isinstance(appsec, dict) else None
    if score is not None:
        try:
            ...
            score = None
    if score is not None:
        # Map score to risk label + colour (mirrors PDF template logic)
        if score < 30:
            label = '极高风险'
            bg = 'C0392B'   # red
        elif score < 40:
        ...
        # Compute letter grade (mirrors template branch)
        if score < 30:
            grade = 'F'
        elif score < 40:
            grade = 'C'
        ...
        run = para.add_run(
            f'应用安全评分   {score} / 100')
        _set_run_font(run, size=14, bold=True, color=(0x1F, 0x4E, 0x79))
        ...
        _set_run_font(run, size=12, bold=True, color=(0xFF, 0xFF, 0xFF))
        _shade_paragraph(para, bg)
        # Append grade on the same line
        run2 = para.add_run(f'    等级：{grade}    ')
        _set_run_font(run2, size=12, bold=True, color=(0x1F, 0x4E, 0x79))

    # ---- Metadata table (file name + scan date + hashes) ----
    meta_rows = [
        ('文件名', context.get('file_name', '—')),
        ('包名', context.get('package_name', '—')),
        ('扫描日期', context.get('timestamp', '—')),
        ('MD5', context.get('md5', '—')),
        ('SHA1', context.get('sha1', '—')),
        ('SHA256', context.get('sha256', '—')),
    ]
    meta_rows = [(k, v) for k, v in meta_rows if v and v != '—']
    if meta_rows:
        para = doc.add_paragraph()
        para.paragraph_format.space_before = Pt(12)
        run = para.add_run('扫描信息')
        _set_run_font(run, size=12, bold=True, color=(0x1F, 0x4E, 0x79))
        ...
        for i, (k, v) in enumerate(meta_rows):
            kc = table.cell(i, 0)
            kc.text = ''
            run = kc.paragraphs[0].add_run(k)
            _set_run_font(run, size=10, bold=True, color=(0x1F, 0x4E, 0x79))
            vc = table.cell(i, 1)
            vc.text = ''
            run = vc.paragraphs[0].add_run(str(v))
    ...
    para.paragraph_format.space_before = Pt(18)
    para.alignment = 1
    run = para.add_run('本报告由 MobSF 自动生成 · 仅供安全评估使用')
    _set_run_font(run, size=10, color=(0x80, 0x80, 0x80))

    # ---- Hard page break before the body ----
    _add_page_break(doc)
...
def html_to_docx(html_str, context=None):
    ...
    # The MobSF PDF templates wrap the report in <article id="cover">.
    # Detach that block from the body and render it via the dedicated
    # cover builder so it gets a one-page treatment, then walk the
    # remaining body content normally.
    cover = soup.find('article', id='cover')
    if cover is not None:
        cover.extract()
        _build_cover_page(doc, context)
